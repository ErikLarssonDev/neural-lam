...
# Inputs
# 0/1 exceedance on coarse grid
ECE_MASK_COARSE = os.path.join(BASE_DIR, "pr_gt_p95_r2_mask.nc")
...
NLAM_TILES_GLOB = os.path.join(
    BASE_DIR, "bias_exceedance_r2", "tiles", "pr", "frac", "pr_p95_exceedance_frac_*.nc")
...
def load_nlam_frac_daily():
    ...
    def preprocess(ds):
        ...
        ds[latn].attrs.pop("bounds", None)
        ds[lonn].attrs.pop("bounds", None)
        return ds
    ds = xr.open_mfdataset(files, combine="by_coords", preprocess=preprocess)
    varname = next(v for v in ds.data_vars if v.endswith(
        "_p95_exceedance_frac"))
...
def interpolate_to_hclim(ece_mask_coarse, hclim_sample):
    ...
    rename = {}
    if "latitude" in ece_mask_coarse.coords and lat_h == "lat":
        rename["latitude"] = "lat"
    if "longitude" in ece_mask_coarse.coords and lon_h == "lon":
        rename["longitude"] = "lon"
    src = ece_mask_coarse.rename(rename)
    out = src.interp(
        {lat_h: hclim_sample[lat_h], lon_h: hclim_sample[lon_h]}, method="nearest")
...
def main():
    ...
    # Align time
    nlam_mask, hclim_mask, ece_mask = align_times(
        nlam_mask, hclim_mask, ece_mask)

    # Confusion & MCCs
    tp_a, fp_a, fn_a, tn_a = per_grid_confusion(
        nlam_mask,  ece_mask)  # NLAM vs ECE
    tp_b, fp_b, fn_b, tn_b = per_grid_confusion(
        hclim_mask, ece_mask)  # HCLIM vs ECE
    ...
    cbar = fig.colorbar(
        ims[0], ax=axes, orientation="horizontal", fraction=0.06, pad=0.12)
    cbar.set_label("MCC")

    out_png = os.path.join(
        OUT_DIR, "fig_MCC_NeuralLAM_and_HCLIM_vs_ECEr2_sharedcbar_cropped.png")
