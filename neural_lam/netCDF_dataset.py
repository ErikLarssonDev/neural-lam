...
class NetCDFDataset(Dataset):
    ...
    def __init__(self, start_date, end_date,
        ...
        if static_fields_files and any(file.strip() for file in static_fields_files):
            self.static_fields_paths = [os.path.join(input_path, file) for file in static_fields_files if file.strip()]
        else:
        ...
        ground_truth_size = self.get_batch(self.ground_truth_datasets, 0).shape[1:]
        ...
        if self.static_fields_paths:
            static_fields = [self.load_static_field(fp) for fp in self.static_fields_paths]
            self.static_data = torch.cat(static_fields, dim=0)  # [C_static, H, W]
    ...
    def get_day_of_year_tensor(self, idx, height, width):
        ...
        sin_tensor = torch.full((1, height, width), fill_value=doy_sin, dtype=torch.float32)
        cos_tensor = torch.full((1, height, width), fill_value=doy_cos, dtype=torch.float32)
