...
from neural_lam.models.encoder import UNet
from neural_lam.models.ar_model import ARModel
from neural_lam import constants
from neural_lam import constants

class SwinUTrans2(ARModel):
    """
    ...
    def __init__(self, args):
        ...
        self.input_dim = 2*constants.GRID_STATE_DIM + constants.GRID_FORCING_DIM #  + constants.BATCH_STATIC_FEATURE_DIM
        ...
        self.unet = UNet(in_channels=self.input_dim, out_channels=self.output_dim)
        self.model = SwinTransformerSys(img_size=(256, 256), in_chans=self.input_dim, num_classes=self.output_dim, window_size=8)
    ...
    def predict_step(self, prev_state, prev_prev_state, forcing):
        ...
        input_grid = torch.reshape(input_flat, (-1, *constants.GRID_SHAPE, input_flat.shape[2])) # (B, N_x, N_y, d_input)
        ...
        input_grid, skip_connections = self.unet.encode(input_grid)
        output_grid = self.model(input_grid) # Shape (B, d_state, N_x, N_y)
        output_grid = self.unet.decode(output_grid, skip_connections)
...
def window_partition(x, window_size):
    ...
    # Standard case H / 4, H / 7 / 7
    # x.shape: torch.Size([1, 56, 56, 1])
    # x.shape: torch.Size([1, 28, 28, 1])
    # x.shape: torch.Size([1, 14, 14, 1])
    # x.shape: torch.Size([1, 14, 14, 1])
    # x.shape: torch.Size([1, 28, 28, 1])
    # x.shape: torch.Size([1, 56, 56, 1])
...
if __name__=='__main__':
    ...
    print('#### Test Model ###')
    x = torch.rand(4, 50, 224, 224).to(device)
    # unet = UNet(in_channels=50, out_channels=17).to(device)
    model = SwinTransformerSys(img_size=(224, 224), in_chans=50, num_classes=17, window_size=7).to(device)
    # x, skip_connections = unet.encode(x)
    # print(x.size())
    y = model(x)
    print(y.size())
    # y = unet.decode(y, skip_connections)
    # print(y.size())