...
class EDMPrecond(torch.nn.Module):
    ...
    def forward(self, x, sigma, class_labels=None, force_fp32=False, **model_kwargs):
        x = torch.reshape(x, (-1, 256, 232, x.shape[2])).permute((0,3,1,2)).contiguous()
        if class_labels is not None:
            class_labels = torch.reshape(class_labels, (-1, 256, 232, class_labels.shape[2])).permute((0,3,1,2)).contiguous()
        ...
        assert F_x.dtype == dtype
        D_x = c_skip * x + c_out * F_x.to(torch.float32)
        return D_x.permute((0,2,3,1)).flatten(1,2)
...
if __name__=='__main__':
    ...
    input_grid = torch.rand(1, 50, 256, 232).to(device)
    model = EDMPrecond(img_resolution=256, in_channels=67, out_channels=17, model_type='SongUNet').to(device)
    # print(model)
    ...
    y = model(noisy_input, sigma, input_grid)
    print(y.size())