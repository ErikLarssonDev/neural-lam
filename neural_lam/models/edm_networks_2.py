...
class SongUNet(torch.nn.Module):
    def __init__(self,
                 ...
                 out_channels,
                 # Number of color channels at input for boundary nodes.
                 in_channels_boundary=None,
                 ...
                 # Whether to remove the conditioning from x at inference with the model.
                 remove_cond=False,
                 ):
        ...
        self.config_loader = config.Config.from_file(
        ...
        # TODO: Make sure that hidden_layers gets passed correctly
        ...
        self.boundary_embedder = utils.make_mlp(
            # TODO: Make this dynamic
            [in_channels_boundary] + self.mlp_blueprint_end, noise_level_dim=emb_channels
        )
    ...
    def forward(self, x, noise_labels, class_labels=None, boundary_forcing=None):
        ...
        grid_emb = self.grid_embedder(
            grid_features, emb)  # (B, num_grid_nodes, d_h)
        ...
        reconstructed_image = torch.zeros(
            (grid_emb.shape[0], constants.FULL_GRID_SHAPE[0] * constants.FULL_GRID_SHAPE[1], grid_emb.shape[2]), device=grid_emb.device)
        ...
        x = reconstructed_image.reshape(
            grid_emb.shape[0], *constants.FULL_GRID_SHAPE, -1).permute(0, 3, 1, 2).contiguous()
        emb = emb.squeeze()
...
class DhariwalUNet(torch.nn.Module):
    def __init__(self,
        ...
        init = dict(init_mode='kaiming_uniform',
                    init_weight=np.sqrt(1/3), init_bias=np.sqrt(1/3))
        init_zero = dict(init_mode='kaiming_uniform',
                         init_weight=0, init_bias=0)
        block_kwargs = dict(emb_channels=emb_channels, channels_per_head=64,
                            dropout=dropout, init=init, init_zero=init_zero)

        # Mapping.
        self.map_noise = PositionalEmbedding(num_channels=model_channels)
        self.map_augment = Linear(in_features=augment_dim, out_features=model_channels,
                                  bias=False, **init_zero) if augment_dim else None
        self.map_layer0 = Linear(
        ...
        self.map_label = Linear(in_features=label_dim, out_features=emb_channels, bias=False,
                                init_mode='kaiming_normal', init_weight=np.sqrt(label_dim)) if label_dim else None
...
class VPPrecond(torch.nn.Module):
    ...
    def forward(self, x, sigma, class_labels=None, force_fp32=False, **model_kwargs):
        ...
        class_labels = None if self.label_dim == 0 else torch.zeros(
            [1, self.label_dim], device=x.device) if class_labels is None else class_labels.to(torch.float32).reshape(-1, self.label_dim)
...
class iDDPMPrecond(torch.nn.Module):
    def __init__(self,
                 ...
                 label_dim=0,
                 use_fp16=False,            # Execute the underlying model at FP16 precision?
                 # Timestep adjustment at low noise levels.
                 C_1=0.001,
                 # Timestep adjustment at high noise levels.
                 C_2=0.008,
                 # Original number of timesteps in the DDPM formulation.
                 M=1000,
        ...
        for j in range(M, 0, -1):  # M, ..., 1
            u[j - 1] = ((u[j] ** 2 + 1) / (self.alpha_bar(j - 1) /
                        self.alpha_bar(j)).clip(min=C_1) - 1).sqrt()
    ...
    def forward(self, x, sigma, class_labels=None, force_fp32=False, **model_kwargs):
        ...
        c_in = 1 / (sigma ** 2 + 1).sqrt()
        c_noise = self.M - 1 - \
            self.round_sigma(sigma, return_index=True).to(torch.float32)
    ...
    def round_sigma(self, sigma, return_index=False):
        sigma = torch.as_tensor(sigma)
        index = torch.cdist(sigma.to(self.u.device).to(torch.float32).reshape(
            1, -1, 1), self.u.reshape(1, -1, 1)).argmin(2)
...
class tEDMPrecond(torch.nn.Module):
    def __init__(self,
                 ...
                 # Number of output channels.
                 out_channels,
                 # The degrees of freedom for the student-t noise.
                 v,
                 # Number of class labels, 0 = unconditional.
                 label_dim=0,
                 use_fp16=False,            # Execute the underlying model at FP16 precision?
    ...
    def forward(self, x, sigma, class_labels=None, boundary_forcing=None, force_fp32=False, **model_kwargs):
        ...
        c_skip = self.sigma_data ** 2 / \
            (((self.v / (self.v - 2)) * sigma ** 2) + self.sigma_data ** 2)
        c_out = torch.sqrt(self.v / (self.v - 2)) * sigma * self.sigma_data / \
            ((self.v / (self.v - 2)) * sigma ** 2 + self.sigma_data ** 2).sqrt()
        c_in = 1 / ((self.v / (self.v - 2)) *
...
if __name__ == '__main__':
    ...
    model = EDMPrecond(img_resolution=torch.tensor([268, 238]), in_channels=71, out_channels=17, model_type='SongUNet', resample_filter=[
                       1, 1], channel_mult=[1, 2, 2, 2], encoder_type='residual').to(device)
    # print(model)
    ...
    rnd_uniform = torch.rand(
        [input_grid.shape[0], 1, 1], device=input_grid.device)
    rho = 7
    sigma_min = 0.02
    sigma_max = 88
    rho_inv = 1 / rho
