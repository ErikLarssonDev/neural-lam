...
class UNetBlock(torch.nn.Module):
    def __init__(self,
        ...
        if up:
            self.conv2 = Conv2d(in_channels=out_channels, out_channels=out_channels, kernel=3, padding=padding, **init)
...
class SongUNet_old(torch.nn.Module):
    def __init__(self,
        ...
        for level, mult in enumerate(channel_mult):
            ...
            if level == 0:
                ...
                self.enc[f'{res}x{res}_down'] = UNetBlock(in_channels=cout, out_channels=cout, down=True, padding=(0,1), **block_kwargs)
...
class SongUNet(torch.nn.Module):
    def __init__(self,
        ...
        model_channels      = 128,          # Base multiplier for the number of channels.
        channel_mult        = [1,2,2,2],    # Per-resolution multipliers for the number of channels. TODO: Try to increase the channels
        channel_mult_emb    = 4,            # Multiplier for the dimensionality of the embedding vector.
        num_blocks          = 4,            # Number of residual blocks per resolution.
        attn_resolutions    = [16],         # List of resolutions with self-attention. TODO: Test to add attention to different resolutions
        dropout             = 0.10,         # Dropout probability of intermediate activations.
        label_dropout       = 0,            # Dropout probability of class labels for classifier-free guidance.

        embedding_type      = 'positional', # Timestep embedding type: 'positional' for DDPM++, 'fourier' for NCSN++.
        channel_mult_noise  = 1,            # Timestep embedding size: 1 for DDPM++, 2 for NCSN++.
        encoder_type        = 'standard',   # Encoder architecture: 'standard' for DDPM++, 'residual' for NCSN++. TODO: Test to change to residual
        decoder_type        = 'standard',   # Decoder architecture: 'standard' for both DDPM++ and NCSN++.
        ...
        for level, mult in enumerate(channel_mult):
            if level == 0:
                ...
                self.enc[f'{res[0]}x{res[1]}_down'] = UNetBlock(in_channels=cout, out_channels=cout, down=True, padding=1, **block_kwargs)
                if encoder_type == 'skip':
                    self.enc[f'{res[0]}x{res[1]}_aux_down'] = Conv2d(in_channels=caux, out_channels=caux, kernel=0, down=True, resample_filter=resample_filter)
                    self.enc[f'{res[0]}x{res[1]}_aux_skip'] = Conv2d(in_channels=caux, out_channels=cout, kernel=1, **init)
                if encoder_type == 'residual':
                    self.enc[f'{res[0]}x{res[1]}_aux_residual'] = Conv2d(in_channels=caux, out_channels=cout, kernel=3, down=True, resample_filter=resample_filter, fused_resample=True, padding=1, **init)
                    caux = cout
            for idx in range(num_blocks):
                cin = cout
                cout = model_channels * mult
                attn = (res[0] in attn_resolutions) # TODO: Check if we maybe want levels instead of resolutions
                print(f"Attn at level {level} with res {res[0]}: {attn}")
                self.enc[f'{res[0]}x{res[1]}_block{idx}'] = UNetBlock(in_channels=cin, out_channels=cout, attention=attn, **block_kwargs)
        ...
        for level, mult in reversed(list(enumerate(channel_mult))):
            if level == len(channel_mult) - 1:
                self.dec[f'{res[0]}x{res[1]}_in0'] = UNetBlock(in_channels=cout, out_channels=cout, attention=True, **block_kwargs)
                self.dec[f'{res[0]}x{res[1]}_in1'] = UNetBlock(in_channels=cout, out_channels=cout, **block_kwargs)
            else:
                res = res*2
                ...
                if res[1] != res_level[level][1]:
                    ...
                    pad_v = 1
                print(f"res_up: {res}")
                self.dec[f'{res[0]}x{res[1]}_up'] = UNetBlock(in_channels=cout, out_channels=cout, up=True, padding=(pad_h, pad_v), **block_kwargs)
            for idx in range(num_blocks + 1):
                cin = cout + skips.pop()
                cout = model_channels * mult
                attn = (idx == num_blocks and res[0] in attn_resolutions) # TODO: Check if we maybe want levels instead of resolutions
                self.dec[f'{res[0]}x{res[1]}_block{idx}'] = UNetBlock(in_channels=cin, out_channels=cout, attention=attn, **block_kwargs)
            if decoder_type == 'skip' or level == 0:
                if decoder_type == 'skip' and level < len(channel_mult) - 1:
    ...
    def forward(self, x, noise_labels, class_labels, boundary_forcing):
        ...
        # Reshape to regular grid [B, C, 268, 238]
        # Create a blank array for the full image
        reconstructed_image = torch.zeros((grid_emb.shape[0], constants.FULL_GRID_SHAPE[0] * constants.FULL_GRID_SHAPE[1], grid_emb.shape[2]), device=grid_emb.device)

        # Fill in the interior and boundary regions
        reconstructed_image[:, self.interior_mask, :] = grid_emb
        reconstructed_image[:, ~self.interior_mask, :] = boundary_emb

        # Reshape to 2D for convolutional layers
        x = reconstructed_image.reshape(grid_emb.shape[0], *constants.FULL_GRID_SHAPE, -1).permute(0, 3, 1, 2).contiguous()
        emb = emb.squeeze()
...
if __name__=='__main__':
    ...
    model = EDMPrecond(img_resolution=torch.tensor([268, 238]), in_channels=71, out_channels=17, model_type='SongUNet', resample_filter=[1, 1], channel_mult=[1,2,2,2], encoder_type='residual').to(device)
    # print(model)

    # true_states = torch.rand(input_grid.shape[0], 17, input_grid.shape[2], input_grid.shape[3]).to(device)

    input_grid = torch.rand(2, 54064, 50).to(device)
    boundary_forcing = torch.rand(2, 9720, 67).to(device)
    true_states = torch.rand(2, 54064, 17).to(device)
    ...
    start.record()
    y = model(noisy_input, sigma, class_labels=input_grid, boundary_forcing=boundary_forcing)
    end.record()
