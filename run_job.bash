...
python3 train_model.py "--model" $MODEL $DIFFUSION_MODEL "--n_workers" 16 $RUN_NAME --batch_size 4 --val_interval 10 --pred_residual --load $NORM_LOSS_RES_GRAPH_FM --epochs 205 --lr 0.0001 # --eval test --subset_ds 1 --sampler heun # --plot_diffusion_steps  # --border_condition

# python3 train_model.py --model diffusion --diffusion_model graph_fm --graph hierarchical --pred_residual --batch_size 10 --subset_ds 1 --eval test --plot_diffusion_steps --load /proj/berzelius-2022-164/users/x_erila/neural-lam/saved_models/graph_fm-diffusion-6x128-10_31_09-8082/last.ckpt --sampler heun
